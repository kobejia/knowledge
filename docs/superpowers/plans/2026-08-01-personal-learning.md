...
**Goal:** Build the repository-local `personal-learning` skill, migrate the existing editorial rules and reader profile into the skill and its configuration, and leave README as project introduction only.

**Architecture:** `SKILL.md` orchestrates interaction and loads focused references. Personal preferences live in root `personal-learning-config.yaml`, initialized from a bundled template and validated by dependency-free Ruby-backed shell scripts. Shell tests verify configuration behavior, skill structure, migration completeness, and README boundaries before behavioral eval prompts are reviewed.
...
**Files:**
- Create: `tests/personal-learning/test-config.sh`
- Create: `tests/personal-learning/test-structure.sh`

- [ ] **Step 1: Write the failing configuration test**

Create a shell test that copies the skill to a temporary repository, asserts initialization creates `personal-learning-config.yaml`, asserts a second initialization refuses to overwrite it, validates the generated file, and verifies malformed or unsupported values fail validation.
...
skill_dir="$PWD/.agents/skills/personal-learning"

test ! -e "$repo_dir/personal-learning-config.yaml"
"$skill_dir/scripts/init-config.sh" "$repo_dir"
"$skill_dir/scripts/validate-config.sh" "$repo_dir/personal-learning-config.yaml"
test -f "$repo_dir/personal-learning-config.yaml"
...
sed 's/known_technical_domains: expert/known_technical_domains: invalid/' \
  "$repo_dir/personal-learning-config.yaml" > "$repo_dir/invalid.yaml"
if "$skill_dir/scripts/validate-config.sh" "$repo_dir/invalid.yaml"; then exit 1; fi
```

- [ ] **Step 2: Write the failing structure test**

Assert all designed skill files exist, YAML frontmatter has `name: personal-learning`, references are routed from `SKILL.md`, root configuration exists, deleted legacy files are absent, and README contains neither the knowledge-map heading nor legacy links.

- [ ] **Step 3: Run tests to verify RED**

Run: `sh tests/personal-learning/test-config.sh && sh tests/personal-learning/test-structure.sh`

Expected: FAIL because `.agents/skills/personal-learning` and its root configuration do not exist.
...
**Files:**
- Create: `.agents/skills/personal-learning/assets/personal-learning-config.template.yaml`
- Create: `.agents/skills/personal-learning/scripts/init-config.sh`
- Create: `.agents/skills/personal-learning/scripts/validate-config.sh`
- Create: `personal-learning-config.yaml`
...
Run: `.agents/skills/personal-learning/scripts/init-config.sh "$PWD"`

Expected: `Created .../personal-learning-config.yaml`
...
```bash
git add .agents/skills/personal-learning/assets .agents/skills/personal-learning/scripts personal-learning-config.yaml
...
**Files:**
- Create: `.agents/skills/personal-learning/SKILL.md`
- Create: `.agents/skills/personal-learning/references/editorial-policy.md`
- Create: `.agents/skills/personal-learning/references/markdown-quality.md`
- Create: `.agents/skills/personal-learning/references/demo-quality.md`
- Create: `.agents/skills/personal-learning/references/exercise-quality.md`
- Create: `.agents/skills/personal-learning/evals/evals.json`
...
Use frontmatter name `personal-learning` and a trigger-only description. Require configuration validation first, then the fixed sequence: understand or disambiguate, ask for one of four levels, and only for technical topics separately ask about Demo and exercises. Route to the editorial and Markdown references always and the two add-on references only when selected.
...
```bash
git add .agents/skills/personal-learning
...
Keep the project title and concise purpose statement. Remove the knowledge map, document links, editorial summary, and legacy-file links. Optionally mention that `personal-learning` assists research and writing, without embedding its manual.
...
```bash
git add README.md EDITORIAL_GUIDE.md READER_PROFILE.md tests/personal-learning/test-structure.sh
...
**Files:**
- Modify if needed: `.agents/skills/personal-learning/**`
- Modify if needed: `tests/personal-learning/**`

- [ ] **Step 1: Run the complete deterministic suite**

Run: `sh tests/personal-learning/test-config.sh && sh tests/personal-learning/test-structure.sh && git diff --check`

Expected: both test scripts PASS and `git diff --check` emits no output.

- [ ] **Step 2: Validate the live configuration directly**

Run: `.agents/skills/personal-learning/scripts/validate-config.sh personal-learning-config.yaml`

Expected: `Valid personal-learning config: personal-learning-config.yaml`
...
```bash
git add .agents/skills/personal-learning tests/personal-learning personal-learning-config.yaml README.md
