...
**Goal:** Localize the visible personal-learn skill guidance into Chinese, remove configurable level recommendations, and tighten the configuration schema without breaking identifiers or paths.

**Architecture:** Keep `personal-learn`, reference paths, scripts, and internal level IDs stable. Move all human-facing skill guidance to Chinese, retain canonical level semantics in `editorial-policy.md`, and make the Ruby-backed validator reject unknown YAML keys while reading UTF-8 explicitly.
...
**Files:**
- Modify: `tests/personal-learn/test-config.sh`
...
Assert `SKILL.md` has a Chinese `description`, Chinese headings for overview/setup/interaction, the ordered headings `### 1. 理解与消歧`, `### 2. 选择内容档位`, and `### 3. 判断主题类型`, plus Chinese titles in all four references. Continue asserting unchanged paths and `name: personal-learn`.
...
**Files:**
- Modify: `personal-learn-config.yaml`
- Modify: `.agents/skills/personal-learn/assets/personal-learn-config.template.yaml`
- Modify: `.agents/skills/personal-learn/scripts/validate-config.sh`
...
```bash
git add personal-learn-config.yaml .agents/skills/personal-learn/assets/personal-learn-config.template.yaml .agents/skills/personal-learn/scripts/validate-config.sh
...
- Modify: `.agents/skills/personal-learn/references/editorial-policy.md`
- Modify: `.agents/skills/personal-learn/references/markdown-quality.md`
- Modify: `.agents/skills/personal-learn/references/demo-quality.md`
- Modify: `.agents/skills/personal-learn/references/exercise-quality.md`
- Modify: `.agents/skills/personal-learn/evals/evals.json`

- [ ] **Step 1: Localize `SKILL.md`**

Translate the description, headings, workflow, routing, and guardrails into concise Chinese. Preserve `name: personal-learn`, paths, commands, filenames, and level IDs. Remove all configured recommendation language. Keep only one-line option summaries; point formal semantics to `editorial-policy.md`.
...
```bash
git add .agents/skills/personal-learn tests/personal-learn
