...
test ! -e "$fixture_dir/personal-learning-config.yaml"
...
cp "$fixture_dir/personal-learning-config.yaml" "$fixture_dir/unknown-field.yaml"
...
cp "$fixture_dir/personal-learning-config.yaml" "$fixture_dir/classification-field.yaml"
...
awk '/^  experience:/ { print "  unknown_profile_field: true" } { print }' \
  "$fixture_dir/personal-learning-config.yaml" > "$fixture_dir/unknown-nested-field.yaml"
...
sed 's/frontend_years: 10/frontend_years: ten/' \
  "$fixture_dir/personal-learning-config.yaml" > "$fixture_dir/invalid-years.yaml"
...
cp "$fixture_dir/personal-learning-config.yaml" "$fixture_dir/invalid-utf8.yaml"
...
echo "PASS: personal-learning configuration contract"
